...
import lineagos.providers.LineageSettings;
...
public abstract class QSTileImpl<TState extends State> implements QSTile, LifecycleOwner, Dumpable {
    ...
    public void secondaryClick(@Nullable View view) {
        mQSLogger.logTileSecondaryClick(mTileSpec, mStatusBarStateController.getState(),
                mState.state);
        handleClick(ACTION_QS_SECONDARY_CLICK, QSEvent.QS_ACTION_SECONDARY_CLICK, H.SECONDARY_CLICK, view);
    }
    ...
    private void handleClick(int category, QSEvent event, int message, View view) {
        ...
        if (!mContext.getSystemService(KeyguardManager.class).isKeyguardLocked() ||
                ...
                LineageSettings.Global.QS_TILES_TOGGLEABLE_ON_LOCK_SCREEN, 1) == 1) {
