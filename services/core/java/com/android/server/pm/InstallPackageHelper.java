        ...
        for (int index = packageSettings.size() - 1; index >= 0; index--) {
            ...
            if (disabledPs == null) {
                ...
                if (ArrayUtils.contains(DEMOTED_SYSTEM_PACKAGES, packageName)) {
                    action = "kept";
                    ((SettingBase) ps).setFlags(ps.getFlags() & ~ApplicationInfo.FLAG_SYSTEM
                                    & ~ApplicationInfo.FLAG_UPDATED_SYSTEM_APP);
                    ((SettingBase) ps).setPrivateFlags(
                ...
                mRemovePackageHelper.removePackageDataLIF(ps, userIds, new PackageRemovedInfo(),
                        flags, false);
            } else {
